...
describe('Terminal Game Timer Cleanup', () => {
  ...
  it('should clean up all timers properly', () => {
    ...
    const setTimeoutMock = setTimeout as unknown as jest.Mock;
    const setTimeoutCallCount = setTimeoutMock.mock.calls.length;
    
    // Clean up all timers
    game.cleanup();
    
    // Check that clearTimeout was called the same number of times as setTimeout
    const clearTimeoutMock = clearTimeout as unknown as jest.Mock;
    expect(clearTimeoutMock.mock.calls.length).toBe(setTimeoutCallCount);
  });
}); 