...
if [ "$1" == "--ascii" ] || [ "$1" == "-a" ]; then
    ...
    # The Python board renderer has been replaced by TypeScript
    # python3 cmd/terminal_game/integrate_board_renderer.py --scenario=full --delay=$DELAY --seed=123
    echo "Board demo has been replaced with a TypeScript implementation."
    echo "Run 'cd web && npm run board-demo' to see the board visualization."
    cd web && npm run board-demo
else
