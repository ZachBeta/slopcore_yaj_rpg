...
func _ready() -> void:
	...
	$VBoxContainer/TerminalGameButton.pressed.connect(_on_terminal_game_button_pressed)
...
func _on_terminal_game_button_pressed() -> void:
	print("Starting terminal interface...")
	# Change scene to the terminal interface scene
	get_tree().change_scene_to_file("res://scenes/terminal_game.tscn")
